...
-------------------------------------------------------------------------------------------
--- Description: Script to calculate fix tariffs dimension                              ---
---                                                                                     ---
--- Parameters:                                                                         ---
...
FROM
    (SELECT
      ...
      FROM
         ...
         FROM
            {{ project.prefix }}gbic_global_bnss.dims_f_tariffs_history
         WHERE
    ...
    (SELECT
     ...
     FROM
        (SELECT
            gbic_op_id,
            tariff_plan_id,
            tariff_plan_des AS des_plan
         FROM
            {{ project.prefix }}gbic_global.gbic_global_dim_f_tariff_plan
         WHERE
            gbic_op_id = ${targetOb} AND
            month='${nominalTime}'
         ) tp
...
----------------
-- Existing records
INSERT OVERWRITE TABLE dims_f_tariffs
...
INSERT OVERWRITE TABLE dims_f_tariffs_history 
PARTITION ( gbic_op_id_pt, month_pt, hist_flag ) 
SELECT 
    old.gbic_op_id,
    old.gbic_tariff_id,
