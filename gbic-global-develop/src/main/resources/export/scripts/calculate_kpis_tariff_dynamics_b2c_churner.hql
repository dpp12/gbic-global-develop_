...
INSERT INTO TABLE kpis_tariff_dynamics_agg_b2c PARTITION ( status='INACTIVE', month='${hiveconf:curdate}', gbic_op_id=${hiveconf:ob} )
...
FROM (
 ...
 (
  ...
  WHERE gbic_op_id=${hiveconf:ob}
    ...
    AND mov_grp_global_id IN (-1,3,4,6,8)
) AS mov 
