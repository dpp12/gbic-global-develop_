/* antig_term.pig
 * ------------------
 * 
 */

in_data = LOAD '/user/gplatform/inbox/esp/ANTIG_TERM/month=$nominalTime/ANTIG_TERM-$nominalTime.csv'
    USING org.apache.pig.piggybank.storage.CSVExcelStorage('|','YES_MULTILINE')
    AS (id_dia_mes: chararray,
        msisdn:     long,
        imei:       long,
        fecha_alta: chararray
        );
        
unique_data   = DISTINCT in_data;
noheader_data = FILTER unique_data   BY id_dia_mes!='ID_DIA_MES';

store_data = FOREACH noheader_data {
     mes = CONCAT(CONCAT(CONCAT(SUBSTRING(id_dia_mes, 0, 4), '-'), CONCAT(SUBSTRING(id_dia_mes, 4, 6), '-')),SUBSTRING(id_dia_mes, 6, 8));
  GENERATE
    'MOVISTAR ESPAÑA'             AS (gbic_op_name: chararray),
     mes                          AS (id_dia_mes:   chararray),
     msisdn                       AS (msisdn:       long),
     imei                         AS (imei:         long),
     REPLACE(fecha_alta,'\\.','') AS (fecha_alta:   chararray), /*Se trata de una fecha, no se trata como un chararray normal*/
     1                            AS (gbic_op_id:   int),
    '$nominalTime'                AS (month:        chararray);
}

STORE store_data INTO 'GBIC_GLOBAL.GBIC_GLOBAL_ESP_ANTIG_TERM'
    USING org.apache.hcatalog.pig.HCatStorer();
