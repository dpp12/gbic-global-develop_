-------------------------------------------------------------------------------
--- NUMBER OF M_LINE_SERVICES ID_SERVICE NOT IN DIM_M_SERVICES TABLE
-------------------------------------------------------------------------------
--- 
--- Description: Gets the number of records of m_line_services with id_service
...
FROM (
    ...
    LEFT OUTER JOIN (
        ...
        FROM gbic_dq_dim_m_services_for_mlineservices
        WHERE gbic_op_id = ${gbic_op_id}
          AND month = '${nominalTime}'
    ) AS dim_serv
