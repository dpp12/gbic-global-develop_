-------------------------------------------------------------------------------
--- NUMBER OF M_LINES TARIFF_PLAN_ID NOT IN DIM_M_TARIFF_PLAN
-------------------------------------------------------------------------------
--- 
--- Description: Gets the number of records of m_lines with tariff_plan_id
...
FROM (
    SELECT
        m_lines.*,
        IF ( tariff.tariff_plan_id IS NOT NULL OR m_lines.tariff_plan_id = '-1', 'ok', 'ko' ) AS join_dim_m_tariff_plan
    FROM (
        SELECT
            tariff_plan_id,
            month,
    ...
    LEFT OUTER JOIN (
      ...
      ON m_lines.tariff_plan_id = tariff.tariff_plan_id
) z
