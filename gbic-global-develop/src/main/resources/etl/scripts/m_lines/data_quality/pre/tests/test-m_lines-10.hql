-------------------------------------------------------------------------------
--- VALIDATE IF THERE ARE RECORDS WITH MSISDN_ID NOT ANONYMIZED
-------------------------------------------------------------------------------
--- 
--- Description: Test if there are records with msisdn_id not anonymized.
--- Parameters:
--- 
---     gbic_op_id:  Country internal identification
---     fileName:    Name of the processed file. 'm_lines' in this case
---     nominalTime: Date of the file in format YYYY-mm-dd
---     test_id:     10 in this case
-------------------------------------------------------------------------------
...
-- SET hivevar:test_id=10;
SET hivevar:screenId=9;
...
FROM (
    SELECT
        ...
        test_expected_value,
        SUM( IF(scResult.fieldContent IS NULL, scResult.fieldValue, IF(fTest.field_eval != scResult.fieldContent, scResult.fieldValue, 0 ))) AS fieldMatchingValue,
