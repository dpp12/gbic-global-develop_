 ...
 * ...
 */
movement_type = LOAD '{{ cluster.service }}/homog/month=$nominalTime/dim=3/*'
    USING PigStorage(',')
...
m_campaign = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_M_CAMPAIGN/month=$nominalTime/*'
    USING PigStorage('|')
...
m_movement = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_M_MOVEMENT/month=$nominalTime/*'
    USING PigStorage('|')
...
m_oper = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_M_OPERATORS/month=$nominalTime/*'
    USING PigStorage('|')
...
in_data = LOAD '{{ hdfs.inbox }}/$ob/$version/MOVEMENTS/month=$nominalTime/*'
    USING PigStorage('|')
    AS (country_id:           int,
        month_id:             chararray,
...
STORE store_data INTO '{{ project.prefix }}gbic_global_staging.gbic_global_movements'
