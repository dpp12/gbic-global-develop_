...
in_data = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_F_TARIFF_PLAN/month=$nominalTime/*'
    USING PigStorage('|')
    AS (country_id:      int,
        month_id:        chararray,
...
STORE store_data INTO '{{ project.prefix }}gbic_global_staging.gbic_global_dim_f_tariff_plan'
