...
f_voice_tariff_plan = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_F_TARIFF_PLAN/month=$nominalTime/*'
    USING PigStorage('|')
...
f_bband_tariff_plan = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_F_TARIFF_PLAN/month=$nominalTime/*'
    USING PigStorage('|')
...
f_tv_tariff_plan = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_F_TARIFF_PLAN/month=$nominalTime/*'
    USING PigStorage('|')
...
in_data = LOAD '{{ hdfs.inbox }}/$ob/$version/F_LINES/month=$nominalTime/*'
    USING PigStorage('|')
    AS (country_id:             int,
        month_id:               chararray,
...
store_data = FOREACH gbic_tv_tariff GENERATE
    ...
    calls_voice_qt                                                AS (calls_voice_qt:        double),
    minutes_voice_qt                                              AS (minutes_voice_qt:      double),
    gbic_op_id                                                    AS (gbic_op_id:            int),
    '$nominalTime'                                                AS (month:                 chararray);

STORE store_data INTO '{{ project.prefix }}gbic_global_staging.gbic_global_f_lines'
