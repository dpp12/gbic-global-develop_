-------------------------------------------------------------------------------
--- RECORDS WITHOUT VOICE_TARIFF_PLAN_ID
-------------------------------------------------------------------------------
--- 
...
SELECT
    'join_dim_f_tariff_plan_voice'             AS field,
    join_dim_f_tariff_plan_voice               AS fieldContent,
    count(*)                                   AS fieldValue,
...
FROM (
    SELECT
        lines.*,
        IF (tariff.f_tariff_plan_id IS NOT NULL OR lines.voice_tariff_plan_id ='-1', 'ok', 'ko') AS join_dim_f_tariff_plan_voice
    FROM (
        SELECT voice_tariff_plan_id,
               month,
               gbic_op_id
    ...
    (
      ...
      AND lines.voice_tariff_plan_id = tariff.f_tariff_plan_id
) AS tariffs
GROUP BY join_dim_f_tariff_plan_voice;
