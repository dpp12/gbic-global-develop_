-------------------------------------------------------------------------------
--- RECORDS WITHOUT TV_TARIFF_PLAN_ID
-------------------------------------------------------------------------------
--- 
...
SELECT
    'join_dim_f_tariff_plan_tv'                AS field,
    join_dim_f_tariff_plan_tv                  AS fieldContent,
    count(*)                                   AS fieldValue,
...
FROM (
    SELECT
        lines.*,
        IF (tariff.f_tariff_plan_id IS NOT NULL OR lines.tv_tariff_plan_id ='-1', 'ok', 'ko') AS join_dim_f_tariff_plan_tv
    FROM (
        SELECT tv_tariff_plan_id,
               month,
               gbic_op_id
    ...
    (
      ...
      AND lines.tv_tariff_plan_id = tariff.f_tariff_plan_id
) AS tariffs
GROUP BY join_dim_f_tariff_plan_tv;
