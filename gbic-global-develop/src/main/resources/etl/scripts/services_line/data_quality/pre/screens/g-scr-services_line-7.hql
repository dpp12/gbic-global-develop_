...
-------------------------------------------------------------------------------
--- NUMBER OF SERVICES_LINE GROUP_SVA NOT IN DIM_M_GROUP_SVA TABLE
-------------------------------------------------------------------------------
--- 
--- Description: Gets the number of records of services_line with group_sva
...
FROM (
    ...
    LEFT OUTER JOIN (
        ...
        FROM gbic_dq_dim_m_group_sva_for_servicesline
        WHERE gbic_op_id = ${gbic_op_id}
          AND month = '${nominalTime}'
    ) AS dim_group
