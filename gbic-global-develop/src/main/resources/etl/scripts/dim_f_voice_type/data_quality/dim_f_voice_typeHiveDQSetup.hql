...
CREATE EXTERNAL TABLE IF NOT EXISTS gbic_dq_dim_f_voice_type_ext (
    country_id                  int,
    month_id                    string,
    voice_type_cd               string,
    voice_type_des              string
) COMMENT 'External table over dim_f_voice_type raw files'
PARTITIONED BY (
    gbic_op_id                  int,
    month                       string
...
CREATE TABLE IF NOT EXISTS gbic_dq_dim_f_voice_type (
    country_id                  int,
    month_id                    string,
    voice_type_cd               string,
    voice_type_des              string
) COMMENT 'ORC table over dim_f_voice_type raw files'
PARTITIONED BY (
    gbic_op_id                  int,
    month                       string
) ROW FORMAT DELIMITED
...
-------------------------------------------------------------------------------
ALTER TABLE gbic_dq_dim_f_voice_type_ext
...
-------------------------------------------------------------------------------
INSERT OVERWRITE TABLE gbic_dq_dim_f_voice_type
PARTITION (
    gbic_op_id = ${gbic_op_id},
    month = '${nominalTime}'
) SELECT
    ...
    voice_type_cd,
    voice_type_des
FROM gbic_dq_dim_f_voice_type_ext
