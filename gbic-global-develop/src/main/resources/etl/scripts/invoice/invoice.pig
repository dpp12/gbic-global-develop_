...
billing = LOAD '{{ hdfs.inbox }}/$ob/$version/DIM_M_BILLING_CYCLE/month=$nominalTime/*'
    USING PigStorage('|')
...
in_data = LOAD '{{ hdfs.inbox }}/$ob/$version/INVOICE/month=$nominalTime/*'
    USING PigStorage('|')
    AS (country_id:         int,
        month_id:           chararray,
...
STORE store_data INTO '{{ project.prefix }}gbic_global_staging.gbic_global_invoice'
