#!/usr/bin/env bash
# 
# gplatform ingestion process
# ---------------------------
# 
# Takes files from ${LOCAL_INBOX_PER_OB} and distributes them by name into HDFS
# It has to be executed as 'javierb' user.
# It has to have write access to local inbox directory, p.e: { inbox }/{OB}/GPLATFORM/{ project.version }
# 
# Usage: (as admin) ** Execute script with --help for details on usage.
# 
# # ${GPLATFORM_HOME}/ingestion/ingest-files.sh {vers-num} {country} {dataset-list} [{yyyyMM} [{local_path_prefix}]]
# 
# Example:
# 
# # ${GPLATFORM_HOME}/ingestion/ingest-files.sh 5 ES "CUSTOMER M_LINES DIM_M_TARIFF_PLAN" 201501
# 
###################################################################################################
...
###################################################################################################
showHelp () {
  echo -e "Usage: $0 [OPTION] MS_VERSION COUNTRY DATASET_LIST [ MONTH [LOCAL_PATH_PREFIX] ]"
...
die () {
  __die "${ALARM_URL}"    \
        ...
        "$2"              \
        "${SERVICE_NAME}" \
        "File ingestion script for '${OB_2M}', '${DATASETS}' and ${CONTENT_DATE}"
}
...
# ARGS: MS_VERSION, OB_2M, DATASETS, CONTENT_DATE (optional) Format yyyyMM, LOCAL_PATH_PREFIX (optional)
###################################################################################################
# Validate MS_VERSION
# -------------------------------------------------------------------------------------------------
MS_VERSION=$1
...
DATASETS=$3
if [[ "$DATASETS" == "" ]]; then
  cancel "wrong Interface list: NOT RECEIVED"
fi
# -------------------------------------------------------------------------------------------------
# Validate MONTH
# -------------------------------------------------------------------------------------------------
...
# {LOCAL_PATH_PREFIX}/{OB3M}/ + GPLATFORM/MSv{VERS}/{YYYY}{MM}/{OB2M}_{INTERFACE}_{YYYY}{MM}.bz2
#                                                                |_ {OB2M}_{INTERFACE}_{YYYY}{MM}.txt
LOCAL_INBOX_PER_OB="GPLATFORM/MSv${MS_VERSION}"
...
# Console messages
debug "Launched ingestion script for MSv${MS_VERSION} on ${EXEC_TIME}"
debug "Logs will be available on ${LOCAL_PATHS_PREFIX}/{country}/${LOGFILE_LOCAL_PATH_PER_OB}/${LOGFILE_NAME}"
debug "OPTIONS:"
debug "ARGUMENTS:"
...
debug "|- Datasets to be ingested: '${DATASETS}'."
if [[ "${MONTH}" == "" ]]; then
  # Console message
  debug "|- Date of file to be ingested not received. Default date will be used (previous month '${CONTENT_DATE}')"
else
  # Console message
  debug "|- Date of file to be ingested: '${CONTENT_DATE}'"
fi
if [[ "${PREFIX}" == "" ]]; then
  # Console message
  debug "|- Root folder for file to be ingested not specified. Default will be used: '${LOCAL_PATHS_PREFIX}'"
else
  # Console message
  debug "|- Root folder for file to be ingested: '${LOCAL_PATHS_PREFIX}'"
fi
...
# Log message
info "Searching files of ${OB_3M} for ${CONTENT_DATE} in ${BZ2_FILE_PATH}. Launched at $EXEC_TIME" > ${LOG_FILE} 2>&1

for INTERFACE in ${DATASETS}; do
    
    # Log message
    info "Processing ${INTERFACE}" >> ${LOG_FILE} 2>&1
    
    HDFS_PATH="${HDFS_INBOX}/${OB_3m}/MSv${MS_VERSION}/${INTERFACE}/month=${CONTENT_YEAR}-${CONTENT_MONTH}-01"
    ...
    BZ2_FILE_GLOB=${BZ2_FILE_PATH}/${OB_2M}_${INTERFACE}_*_${CONTENT_DATE}.bz2
    NUM_FILES=`ls ${BZ2_FILE_GLOB} 2> /dev/null | wc -l`
    if [ "${NUM_FILES}" -eq "0" ]; then
        ERR_MSG="File(s) not found"
        # Log message
        error "${ERR_MSG} for ${OB_3M}'s ${INTERFACE} on ${CONTENT_YEAR}-${CONTENT_MONTH}-01" >> ${LOG_FILE} 2>&1
    ...
    # Log message
    info "${INTERFACE} processed" >> ${LOG_FILE} 2>&1
done

log_file_hdfs=${LOGFILE_HDFS_PATH}/${OB_3M}-${LOGFILE_NAME}
# Log message
info "Finished processing files of ${OB_3M}. Log file will be available on HDFS: ${log_file_hdfs}" >> ${LOG_FILE} 2>&1
