...
group_sva = LOAD '/user/gplatform/inbox/{esp}/MSv5/DIM_M_GROUP_SVA/month=2015-01-01/*'
    USING PigStorage('|')
...
services = LOAD '/user/gplatform/inbox/{esp}/MSv5/DIM_M_SERVICES/month=2015-01-01/*'
    USING PigStorage('|')
...
in_data = LOAD '/user/gplatform/inbox/{esp}/MSv5/SERVICES_LINE/month=2015-01-01/*'
    USING PigStorage('|')
    AS (country_id:       int,
        month_id:         chararray,
        msisdn_id:        chararray,
...
STORE store_data INTO 'gbic_global_staging.gbic_global_services_lines'
