...
tac_file = LOAD '/user/gbic/common/tacs/month=2015-05-01/*'
...
m_tariff_plan = LOAD '/user/gplatform/inbox/{bra}/MSv5/DIM_M_TARIFF_PLAN/month=2015-05-01/*'
    USING PigStorage('|')
...
in_data = LOAD '/user/gplatform/inbox/{bra}/MSv5/M_LINES/month=2015-05-01/*'
    USING PigStorage('|')
    AS (country_id:         int,
        month_id:           chararray,
        msisdn_id:          chararray,
...
gbic_global_data_tac = JOIN
    gbic_global_data_segment BY (SUBSTRING(imei_num, 0, 8)) LEFT OUTER,
...
store_data = FOREACH gbic_global_data_tariff {
    technology = (tac IS NULL?
                    'UNKNOWN':
                    (technology_4g == 'No LTE'? (technology_3g == 'No'? '2G': '3G'): '4G')
                 );
  GENERATE
...
STORE store_data INTO 'GBIC_GLOBAL.GBIC_GLOBAL_M_LINES'
