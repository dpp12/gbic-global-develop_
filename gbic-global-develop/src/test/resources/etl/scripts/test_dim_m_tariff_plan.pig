...
in_data = LOAD '/user/gplatform/inbox/{bra}/MSv5/DIM_M_TARIFF_PLAN/month=2015-06-01/*'
    USING PigStorage('|')
    AS (country_id:      int,
        month_id:        chararray,
...
store_data = FOREACH gbic_filter_obs {
  GENERATE
    ...
    des_plan                AS (des_plan:        chararray),
    data_tariff_ind         AS (data_tariff_ind: int),
    gbic_op_id              AS (gbic_op_id:      int),
    '2015-06-01'            AS (month:           chararray);
}

STORE store_data INTO 'GBIC_GLOBAL.GBIC_GLOBAL_DIM_M_TARIFF_PLAN'
